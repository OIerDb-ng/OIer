...
] + [i * 0.01 for i in list(range(750, 0, -5))]
sc_rt = {
	"NOI": 1,
	"NOID类": 0.75,
	"CTSC": 0.6,
...
with open("data.txt") as source:
	for i in source:
		...
		school_info[schid]["rating"] += sc[max(
			int(crk * 390 / dp[cname]),
			0)] * sc_rt[ctype] * (0.8**(datetime.datetime.now().year - year))
#print(dp)
...
for i in school_info:
	...
	for kk in range(12):
		if i["rating"] > rkreq[kk]:
			cr = rk[kk]
			break
	f.write('"' + str(i["id"]) + '","' + dmp(
		sorted(
			i["name"], key=lambda x: recy[x] * 10000 + recd[x],
			reverse=True)) + '","' + dmp(i["awards"]) + '","' +
			str(int(i["rating"] * 10)) + '","' + cr + '","' + i["prov"] +
			'","' + i["city"] + '","' + str(count) + '"\n')
	count += 1
