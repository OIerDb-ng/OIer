...
grades = {"高二":5,"初三":3,"高三":6,"高一":4,"初二":2,"中六":6,
	"中五":5,"初一":1,"初四":4,"中四":4,"高二年级":5,"中一":1,"中三":3,"六年级":0,"五年级":-1,"小六":0}
sex = {"男":1,"女":-1,"@":0}
st = time.time()
school_id = {}
...
with open("data.txt") as source:
	for i in source:
		...
		if cur[3] in grades.keys():
			grade = grades[cur[3]]
		cur = {"identity":cname,"ctype":contests[cname]["ctype"],"award_type":cur[1][len(cur[0]):],"name":cur[2],"grade":grade,
"school":cur[4],"school_id":school_id[cur[4]],"score":cur[5],"province":cur[6],"sex":sex[cur[7]],"rank": 1}
...
for i in contests:
	...
	for j in contests[i]["participants"]:
		...
		if bestu!=-1:
			...
			all_oiers[cname].append({"name":cname,"pinyin":piny,"sex":0,"year":cyear,"records":[j]})
...
for i in all_oiers:
	for j in all_oiers[i]:
		...
		n.write(str(id)+","+j["name"]+",,,"+j["pinyin"]+',,"'+json.dumps(j["records"], encoding="UTF-8", ensure_ascii=False).replace('"',"'")+'",'+str(j["sex"])+",,"+str(j["year"]))
		id+=1
		n.write("\n")
