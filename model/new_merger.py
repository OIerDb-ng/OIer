...
sc = list(range(100, 39, -1)) + [
	i * 0.01 for i in list(range(3600, 750, -15))
] + [i * 0.01 for i in list(range(750, 150, -3))]
sc_rt = {
	"NOI": 1,
	"NOID类": 0.75,
	"CTSC": 0.2,
...
def output():
	...
	for j in final_output_data:
		...
		j = sorted(
			j,
			key=lambda i: i['year'] + contest_date[i['ctype']],
			reverse=True)
		for k in j:
			del k["name"]
			#del k["cal_y"]
...
with open("data.txt") as source:
	for i in source:
		...
		if cur[3] in grades.keys():
			...
			try:
				grade = contests[cname]["year"] - int(
					re.findall(r"[0-9]{4}", cur[3],
							   re.MULTILINE)[0]) + 1 + 3 * ("高" in cur[3])
			except:
				print(cur)
		try:
			cur = {
				...
				"grade": cur[3],
				"school": cur[4].strip(),
				"school_id": school_id[cur[4].strip()],
				"score": cur[5],
				"province": cur[6],
				"sex": sex[cur[7]],
				"rank": 1,
				"year": contests[cname]["year"],
				"rule": hash(cur[8])
			}
		except:
			print(i)
		...
		if grade == 10000:
			cur["cal_y"] = cur["year"] - general[cur["ctype"]] - (
				"NOIP" not in cur["ctype"])
...
for each_n in awd_by_name:
	globeid = 0
	while 1:
		awd_by_name[each_n] = sorted(
			awd_by_name[each_n], key=lambda i: oi_year(i[0]))
...
final_output_data = sorted(
	final_output_data,
	key=lambda i: sum([
		sc[int(j['rank'] * 400 / cnts[j['identity']])] * sc_rt[j['ctype']] * (
			0.8**(datetime.datetime.now().year - j['year'])) for j in i[:-2]
	]),
	reverse=True)
output()
